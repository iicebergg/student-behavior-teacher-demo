...
function report(name: string, overrides: Partial<Settings>): void {
  ...
  const blankToBlank =
    (a.transitions.get('BF', 'BF').count +
      a.transitions.get('BF', 'BS').count +
      a.transitions.get('BS', 'BF').count +
      a.transitions.get('BS', 'BS').count) /
    Math.max(
      1,
      (a.transitions.rowTotals.get('BF') ?? 0) + (a.transitions.rowTotals.get('BS') ?? 0),
    ) /
...
report('flat difficulty', {
  topics: DEFAULT_SETTINGS.topics.map((t) => ({ ...t, difficultyWeight: 0.4 })),
});
...
const same =
  JSON.stringify(a1.attempts.map((x) => x.path)) === JSON.stringify(a2.attempts.map((x) => x.path));
...
const missing = d.attempts.filter((att) =>
  [...att.byQuestion.values()].some((o) => o.visits.length === 0),
);
